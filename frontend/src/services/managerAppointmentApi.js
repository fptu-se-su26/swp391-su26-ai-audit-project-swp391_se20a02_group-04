...
async function managerAppointmentRequest(path, options = {}) {
...
export function getManagerAppointments(params = {}) {
  return managerAppointmentRequest(withQuery("/manager/appointments", params));
}

export function getManagerAppointmentStatistics(params = {}) {
  return managerAppointmentRequest(withQuery("/manager/appointments/statistics", params));
}

export function getManagerAppointmentById(appointmentId) {
  return managerAppointmentRequest(`/manager/appointments/${appointmentId}`);
}

export async function getMappedManagerAppointmentById(appointmentId) {
  const response = await getManagerAppointmentById(appointmentId);
  return mapManagerAppointment(response.data?.appointment || {});
}

export async function updateManagerAppointmentStatus(appointmentId, status, notes = "") {
  const response = await managerAppointmentRequest(`/manager/appointments/${appointmentId}/status`, {
    method: "PUT",
    body: JSON.stringify({
      status,
      ...(notes ? { notes } : {}),
...
export async function startManagerAppointment(appointmentId) {
  return managerAppointmentRequest(`/manager/appointments/${appointmentId}/start`, {
    method: "PUT",
    body: JSON.stringify({}),
  });
}

export async function updateManagerAppointment(appointmentId, payload = {}) {
...
export async function cancelManagerAppointment(appointmentId, reason = "Hủy từ trang quản trị") {
  return managerAppointmentRequest(`/manager/appointments/${appointmentId}`, {
    method: "DELETE",
...
export { managerAppointmentRequest, withQuery };
